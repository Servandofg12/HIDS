...
def exportHashedFiles():
    """ Params: NONE """
    """ Return: NONE """
    """ Comprueba las rutas que hemos indicado en el archivo de configuraci�n y carga todos los archivos de cada una
    de ellas gracias a la funcion anterior 'folderHash', una vez hecho esto crea un archivo 'hashes.hash' si no lo hay y escribe
...
def calculateHashedFiles():
    """ Params: NONE """
    """ Return: NONE """
    """ Calcula los hashes de los archivos nuevamente, y reutilizamos el diccionario creado al principio 'filesAndHashes' esto servira
...
def graph():
    """ Params: NONE """
    """ Return: NONE """
    """ Muestra una grafica en el navegador en base a los datos de las dos listas 'badIntegrity' y 'graphDate' """
    layout_title = "Evolucion de la integridad de los archivos fecha:  " + \
...
def gui():
    ...
    labelGraph = tk.Label(window, text="Abrir grafico ")
    labelConf = tk.Label(window, text="Fichero de configuracion")
    ...
    btnIniciar = tk.Button(window, text="Iniciar", command=initExam)
    ...
    btnGuardar = tk.Button(
        window, text="Guardar configuracion", command=exportConfig)
